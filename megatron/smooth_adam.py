...
class FusedAdamSmooth(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=1e-3, bias_correction=True,
        ...
        super(FusedAdamSmooth, self).__init__(params, defaults)
        self.idset = set()
        ...
        for k in self.state.keys():
            self.idset.add(id(k))
        print(f'THE STATE OF THE UNION: {len(self.idset)}')
    ...
    def step(self, p=None, closure=None, grads=None, output_params=None, scale=None, grad_norms=None, grad_scaler=None):
        ...
        if p == None:
            ...
            for k in self.state.keys():
                idset.add(id(k))
            print(f'THE STATE OF THE UNION: {len(idset)}')
            return loss
        ...
        # TODO: group size always 1 (for now)
        for group in self.param_groups: # [{'params':[None, None, None, ...]}, {...}, ...]
            if len(group['params']) == 0:
                continue
            device = group['params'][0].device
